...
|**vbObject**|9|Has either of the following forms:<br><table><tr><td>i) A (VBA) [object type](../../glossary/vbe-glossary.md#object-type) with a particular [_interface_](../../Glossary/vbe-glossary.md#interface) chosen[<sup>&para;</sup>](#paragraphfootnote "Chosen interface is meant to refer to a defined interface, of the defined interfaces referenced in a COM object's class definition as being implemented by the class (object's interfaces), that is chosen during run-time, & that has to be chosen before conventional execution of any of the object's methods or conventional access of any of the object's properties. If an interface needs to be chosen for an object, and it isn't to be the object's default interface, one of the object's other interfaces has to be programmatically selected (chosen) instead. Programmatic interface selection is fundamentally supported by the VBA language grammar through the use of implicit type casts"), where the chosen interface directly exposes COM's **IDispatch** interface. <sup>[&Dagger;](#doubledaggerfootnote "That the object type implements IDispatch means that the VBA COM object/class that encompasses the object-based data, is an (OLE) Automation object/class. That the chosen interface exposes IDispatch means that the particular object type of the data, can be directly used with (OLE) Automation technology. Object references of such types can be cast to the Object data type if they are not already of the type.")</sup></td></tr><tr><td>ii) [**Nothing**](../../reference/user-interface-help/nothing-keyword.md) value <sup>_(special value)_</sup> when not stored in an object-type variable.</sup></td></tr></table>||VT_DISPATCH|
...
|**vbDataObject**|13|A (VBA) object type with a particular _interface_ chosen[<sup>&para;</sup>](#paragraphfootnote "Chosen interface is meant to refer to a defined interface, of the defined interfaces referenced in a COM object's class definition as being implemented by the class (object's interfaces), that is chosen during run-time, & that has to be chosen before conventional execution of any of the object's methods or conventional access of any of the object's properties. If an interface needs to be chosen for an object, and it isn't to be the object's default interface, one of the object's other interfaces has to be programmatically selected (chosen) instead. Programmatic interface selection is fundamentally supported by the VBA language grammar through the use of implicit type casts"), that is not represented by the **vbObject** constant documented in this table. <sup>[&sect;](#sectionfootnote "An object of such an object type, like all VBA objects, is still a COM object. Like all COM objects and interfaces, such objects expose COM's IUnknown interface. Not to be confused with ActiveX Data Objects (ADO) which is a database technology.")</sup>|VT_UNKNOWN|
...
<table>
 ...
 That the object type implements **IDispatch** means that the VBA COM [object](../../glossary/vbe-glossary#object)/[class](../../glossary/vbe-glossary#class) that encompasses the object-based data, is an (OLE) [Automation object/class](../../Glossary/vbe-glossary.md#automation-object). That the **chosen** _interface_[<sup>&para;</sup>](#paragraphfootnote "Chosen interface is meant to refer to a defined interface, of the defined interfaces referenced in a COM object's class definition as being implemented by the class (object's interfaces), that is chosen during run-time, & that has to be chosen before conventional execution of any of the object's methods or conventional access of any of the object's properties. If an interface needs to be chosen for an object, and it isn't to be the object's default interface, one of the object's other interfaces has to be programmatically selected (chosen) instead. Programmatic interface selection is fundamentally supported by the VBA language grammar through the use of implicit type casts") exposes **IDispatch** means that the particular object type of the data, can be directly used with (OLE) Automation technology. <sup>[&Vert;](#doubleverticalbarfootnote)</sup><BR>Object references of such types can be [cast](../../Reference/User-Interface-Help/data-type-summary.md#implicit-conversions--casts) to the [**Object**](../../reference/user-interface-help/object-data-type.md) data type if they are not already of the type.</td></tr>
