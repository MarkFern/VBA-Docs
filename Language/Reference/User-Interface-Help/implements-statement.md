...
> [!NOTE]
> In respect to object-oriented programming (OOP), Visual Basic for Applications (VBA) doesn't support implementation inheritance (in accordance with COM's specification). Interface inheritance including multiple interface inheritence can, of a sort, be mimicked using the VBA language through the use of the **Implements** statement (although strictly speaking it is interface implementation rather than interface inheritance). Even though multi-level interface inheritance is supported within type libraries, VBA only recognises type-library interfaces that directly inherit from **IDispatch** or **IUnknown**. This, coupled with the limitations of the **Implements** statement, renders multi-level interface inheritance support within VBA not so strong.
